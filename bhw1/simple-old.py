    ...
    for file_path in file_paths:
      with open(file_path, 'r') as f:
        for story_dict in json.load(f):
          self.tokenized_stories.append(self.tokenizer.encode_as_ids(story_dict['story']))
...
vocab_size = 4096
dataset = SimpleStoryDataset([f'dataset/data0{i}.json' for i in range(10)], 'spm-4k', vocab_size)
dataloader = DataLoader(dataset, batch_size=16, pin_memory=True, num_workers=24, shuffle=True, collate_fn=collate_fn)

device = 'cuda' or Suck
torch.backends.cudnn.deterministic = False
torch.backends.cudnn.benchmark = True
model = Transformer(vocab_size, num_layers=8, num_heads=8, d_model=512).to(device)  # 512
